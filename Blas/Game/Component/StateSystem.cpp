...
// 태그 관리
void StateSystem::AddTag(EStateTag tag)
{
...
void StateSystem::AddTagUnique(EStateTag tag)
{
	if (HasTag(tag))
		return;
...
void StateSystem::RemoveTag(EStateTag tag)
{
	EStateTag oldTags = currentTags;
...
void StateSystem::RemoveTagAll(EStateTag tag)
{
	EStateTag oldTags = currentTags;
...
// 태그 검사
bool StateSystem::HasTag(EStateTag tag) const
{
	return (currentTags & tag) == tag;
}

bool StateSystem::HasAllTags(EStateTag tags) const
{
	return (currentTags & tags) == tags;
}

bool StateSystem::HasAnyTag(EStateTag tags) const
{
	return (currentTags & tags) != Tag_None;
}

// 디버그
wstring StateSystem::GetStateTagString() const
{
	wstring result;
...
// Component 인터페이스
void StateSystem::ComponentInit()
{
	currentTags = Tag_None;
}
