...
Vec2 CameraManager::WorldToScreenPoint(Vec2 worldPoint)
{
	// 가상 해상도 기준으로 좌표 변환
	Vec2 virtualSize = SINGLE(EngineInstance)->GetVirtualSize();
...
Vec2 CameraManager::ScreenToWorldPoint(Vec2 screenPoint)
{
	// 가상 해상도 기준으로 좌표 변환
	Vec2 virtualSize = SINGLE(EngineInstance)->GetVirtualSize();
	Vec2 center = virtualSize * 0.5f;
...
void CameraManager::Scroll(Vec2 dir, float velocity)
{
...
void CameraManager::FadeIn(float duration)
{
	targetBright = 1;
	timeToBright = duration;
}

void CameraManager::FadeOut(float duration)
{
	targetBright = 0;
	timeToBright = duration;
}

void CameraManager::SetTargetPos(const Vec2& targetPos, float timeToTarget)
{
...
void CameraManager::BrightToTarget()
{
	timeToBright -= DT;
...
void CameraManager::SetZoom(float zoom, float duration)
{
	targetZoom = zoom;
	timeToZoom = duration;
...
void CameraManager::ZoomToTarget()
{
	timeToZoom -= DT;
...
void CameraManager::Shake(const FShakeParams& params)
{
	shakeParams = params;
