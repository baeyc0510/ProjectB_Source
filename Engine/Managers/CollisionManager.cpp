...
void CollisionManager::CheckLayer(UINT left, UINT right)
{
	layerMask[left][right] = true;
	layerMask[right][left] = true;
}

void CollisionManager::UnCheckLayer(UINT left, UINT right)
{
	layerMask[left][right] = false;
	layerMask[right][left] = false;
}

void CollisionManager::ResetCheck()
{
	for (UINT i = 0; i < layerMask.size(); i++)
		layerMask[i].fill(false);
}

void CollisionManager::CollisionUpdate(UINT left, UINT right)
{
	for (Collider* leftCollider : colliderList[left])
	{
		for (Collider* rightCollider : colliderList[right])
		{
			// 자기 자신과의 충돌을 무시
...
UINT64 CollisionManager::CollisionID(UINT leftID, UINT rightID)
{
	UINT64 result = 0;
	if (leftID < rightID)
	{
...
bool CollisionManager::CheckCollision(Collider* left, Collider* right)
{
	...
	{
		return BoxVsBox(static_cast<BoxCollider*>(left), static_cast<BoxCollider*>(right));
	}
	else if (leftType == EColliderType::Box && rightType == EColliderType::Line)
	{
		return BoxVsLine(static_cast<BoxCollider*>(left), static_cast<LineCollider*>(right));
	}
	else if (leftType == EColliderType::Line && rightType == EColliderType::Box)
	{
		return BoxVsLine(static_cast<BoxCollider*>(right), static_cast<LineCollider*>(left));
	}
	else if (leftType == EColliderType::Line && rightType == EColliderType::Line)
	{
		return LineVsLine(static_cast<LineCollider*>(left), static_cast<LineCollider*>(right));
	}

	return false;
}

bool CollisionManager::BoxVsBox(BoxCollider* a, BoxCollider* b)
{
	Vec2 posA = a->GetPos();
	Vec2 posB = b->GetPos();
...
bool CollisionManager::BoxVsLine(BoxCollider* box, LineCollider* line)
{
...
bool CollisionManager::LineVsLine(LineCollider* a, LineCollider* b)
{
	// 선분-선분 충돌은 현재 미지원
	return false;
}

vector<HitResult> CollisionManager::BoxTrace(const Vec2& center, const Vec2& halfSize, UINT targetLayer, bool bDrawDebug)
{
	vector<HitResult> results;

	for (Collider* collider : colliderList[targetLayer])
	{
...
void CollisionManager::DrawDebugBox(const Vec2& center, const Vec2& halfSize, COLORREF color, float duration)
{
