...
void LineCollider::SetLine(const Vec2& start, const Vec2& end)
{
	localStart = start;
	localEnd = end;
}

Vec2 LineCollider::GetScale() const
{
	// 선분의 바운딩 박스 크기 반환
...
void LineCollider::SetScale(const Vec2& scale)
{
	// 라인 콜라이더는 SetLine으로 설정하므로 무시
}

Vec2 LineCollider::GetWorldStart() const
{
	return GetPos() + localStart;
}

Vec2 LineCollider::GetWorldEnd() const
{
	return GetPos() + localEnd;
}

float LineCollider::GetYAt(float worldX) const
{
...
float LineCollider::GetXAt(float worldY) const
{
	Vec2 start = GetWorldStart();
	Vec2 end = GetWorldEnd();
...
bool LineCollider::IsInXRange(float worldX) const
{
	Vec2 start = GetWorldStart();
	Vec2 end = GetWorldEnd();
...
bool LineCollider::IsPointOnLine(const Vec2& point, float toleranceY) const
{
...
float LineCollider::GetSlopeAngle() const
{
	Vec2 start = GetWorldStart();
	Vec2 end = GetWorldEnd();
