...
void Map::Load(const wstring& mapJsonPath)
{
	Release();
	mapPath = mapJsonPath;
...
void Map::RenderBackground(Vec2 cameraPos)
{
...
void Map::RenderForeground(Vec2 cameraPos)
{
	// 전경 레이어들 렌더링
	for (auto& layer : foregroundLayers)
	{
...
Vec2 Map::GetSpawnPoint(int spawnId) const
{
...
CheckpointData* Map::GetCheckpoint(int id)
{
...
void Map::ActivateCheckpoint(int id)
{
...
const SceneTransitionData* Map::GetTransitionAt(Vec2 pos) const
{
...
wstring Map::GetMapDirectory(const wstring& jsonPath)
{
...
vector<const BoxColliderData*> Map::GetBoxCollidersWithTag(const string& tag) const
{
...
vector<const SlopeColliderData*> Map::GetSlopeCollidersWithTag(const string& tag) const
{
