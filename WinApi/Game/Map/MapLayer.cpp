...
void MapLayer::Load(const wstring& imagePath, float parallaxFactor, Vec2 offsetValue, bool transparent)
{
	Release();
...
void MapLayer::Render(Vec2 cameraPos)
{
	if (!layerImage)
		return;
...
UINT MapLayer::GetWidth() const
{
	return layerImage ? layerImage->GetBmpWidth() : 0;
}

UINT MapLayer::GetHeight() const
{
